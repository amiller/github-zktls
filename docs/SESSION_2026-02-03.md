...
2. **Witness Generator** (`zk-github-attestation/js/`)
   ...
   - Decodes DER ECDSA signature → (r, s)
...
| Component | Status |
|-----------|--------|
| P-256 leaf signature | ✅ Working |
| P-384 cert chain | ✅ Working (in circuit) |
| Witness generation | ✅ Working |
| ZK proof generation | ✅ **WORKING!** |

### Session 5: TOOLCHAIN RESOLVED! 🎉
...
3. Patched local sha512 lib (`std::wrapping_add` → `h[j].wrapping_add(c[j])`)
...
| nargo | bb.js | Result |
|-------|-------|--------|
| 1.0.0-beta.5 | 0.84.0 | ✅ Works for P-256 only circuit |
| 1.0.0-beta.5 | 0.84.0 | ❌ zkpassport deps fail to compile (macro issues) |
| 1.0.0-beta.18 | 0.82.2 | ❌ WASM unreachable error |
| 1.0.0-beta.18 | 4.0.0-nightly.20260131 | ❌ sha512 lib uses old `std::wrapping_add` |
| **1.0.0-beta.17** | **3.0.0-rc.5** | ✅ **WORKS!** |
...
- [x] ~~Resolve toolchain version alignment for proving~~ ✅ Done!
...
2. **Build TypeScript input generator** - Parse Sigstore bundle → circuit witness
...
- [x] Get Fulcio intermediate CA public key → `docs/fulcio-intermediate-ca.md`
- [ ] Create `zk-github-attestation/` directory with Noir circuit
- [x] TypeScript witness generator for Sigstore bundles → `zk-github-attestation/js/`
