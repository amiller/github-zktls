...
> **Important:** The `--ref v1.0.3` flag is required. The faucet contract verifies the exact commit SHA that produced the proof. Running from `master` will generate a valid-looking proof that reverts on-chain with `WrongCommit`.

Or via GitHub UI: **Actions → GitHub Identity → Run workflow** (select `v1.0.3` from the "Use workflow from" dropdown)
...
**"WrongCommit"**
The proof was generated from the wrong git ref. The faucet contract requires proofs from a specific commit SHA (currently tag `v1.0.3`). Re-run the workflow with `--ref v1.0.3` or select the tag in the GitHub UI dropdown.
